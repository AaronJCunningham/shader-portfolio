...
Command: npx gltfjsx@6.2.16 dragon.glb 
...
export function Dragon({pointer}) {
  const group = useRef()
  const { nodes, materials, animations, scene } = useGLTF('/glbs/dragon.glb')
  const { actions } = useAnimations(animations, group)

  useFrame((state, delta) => {
    if(group.current && pointer){
  
      group.current.rotation.y = MathUtils.damp(group.current.rotation.y, (pointer.x* Math.PI) / 6, 2.75, delta);
      group.current.rotation.x = MathUtils.damp(group.current.rotation.x, (-pointer.y * Math.PI) / 12, 2.75, delta);
    }
  ...
  return (
    <group ref={group}  dispose={null} scale={0.2} position={[0,-1,-2.2]} rotation={[0,0.9,0]}>
      <group name="Sketchfab_Scene">
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]} scale={0.002}>
          <group name="aeeb858164ee4e0884da4001c74b8628fbx" rotation={[Math.PI / 2, 0, 0]}>
            <group name="Object_2">
              <group name="RootNode">
                <group name="Hemi" position={[4018.555, 4666.064, 3759.082]} rotation={[-2.595, 0.261, 0.387]} scale={100}>
...
useGLTF.preload('/glbs/dragon.glb')
